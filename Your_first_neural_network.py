...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate_in):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            delta_weights_i_h += self.learning_rate * np.dot(X[:, None], hidden_error_term[None, :])
            # Weight step (hidden to output)
            delta_weights_h_o += self.learning_rate * np.dot(hidden_outputs[:, None], output_error_term[None, :])

        # TODO: Update the weights - Replace these values with your calculations.
        self.weights_hidden_to_output += delta_weights_h_o / len(features) # update hidden-to-output weights with gradient descent step
        self.weights_input_to_hidden += delta_weights_i_h / len(features) # update input-to-hidden weights with gradient descent step
...
st = 20
plt.plot(losses['train'][st:], label='Training loss')
plt.plot(losses['validation'][st:], label='Validation loss')
plt.legend()
_ = plt.ylim()
